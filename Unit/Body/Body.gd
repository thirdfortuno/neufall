...
func update_sprite(grid_units, unit):
	...
	if adjacents.has("up") and adjacents["up"] == unit:
		index = index + 1
	if adjacents.has("right") and adjacents["right"] == unit:
		index = index + 2
	if adjacents.has("down") and adjacents["down"] == unit:
		index = index + 4
	if adjacents.has("left") and adjacents["left"] == unit:
		index = index + 8
